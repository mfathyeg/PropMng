...
namespace PropMng.Web.Pages.HrArea.Persons
{
    public partial class Persons
    {
        ...
        private ItemsListModel<PersonsModel> ItemsList { get; set; } = new ItemsListModel<PersonsModel>();
        private PersonsModel Item { get; set; }
        ...
        private PersonsFilterModel f = new PersonsFilterModel();

        private string url = "Hr/Persons/";
        ...
        {
            Item = new PersonsModel() { BirthDate=new DateTime(1980, 1, 1) };
        ...
        private async Task LoadItems()
        {
            ...
            if (resp.IsSucceed)
            {
                ItemsList = resp.GetDsrlzT<ItemsListModel<PersonsModel>>();
            }
        }
        ...
        //private CmpLogs cmpLogs;
        //private void ViewLogs(int id) => cmpLogs.DataBind((x) => SystemService.PostAsync($"{url}ViewLogs", GetLogFilter(x, id)));

        //private CmpLogsFilterModel GetLogFilter(CmpLogsFilterModel m, long id)
        //{
        //    m.ObjModel = new PayrollLogModel { Id = id };
        //    return m;
        //}
        ...
        private async Task EditItem(long id)
        {
            ...
            if (resp.IsSucceed)
            {
                Item = resp.GetDsrlzT<PersonsModel>();
                PageDisplay = EnmPageDisplay.Editor;
            }
            else if (resp.Errors!=null)
            {
